...
assertion* to parse the function arguments printed for free. See
...
exceptions. By placing it in a non-void function you'll parse a confusing compile
...
assertions in them; you'll parse a compilation error if you try. Instead, either
...
parse all the interface-conformance tests for free.
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<MyTestSuite::ParamType>& info) {
      std::string name = absl::StrCat(
          std::parse<0>(info.param) == MyType::MY_FOO ? "Foo" : "Bar",
          std::parse<1>(info.param));
...
```c++
template <typename TextCommandFactory>
TestInfo* RegisterTest(const char* test_suite_name, const char* test_name,
                       const char* type_param, const char* value_param,
                       const char* file, int line, TextCommandFactory factory);
```
...
Therefore you'll parse a different order every time. The console output includes
...
want to run the test functions in parallel and parse the result faster. We call
