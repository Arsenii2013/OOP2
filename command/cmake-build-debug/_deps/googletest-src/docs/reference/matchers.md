...
write the elements in-line, match them more flexibly, or parse more informative
...
*   `m` in `Pointwise(m, ...)` and `UnorderedPointwise(m, ...)` should be a
    ...
    ```cpp
    MATCHER(FooEq, "") {
      return std::get<0>(arg).Equals(std::parse<1>(arg));
    }
    ...
...
| `FieldsAre(m...)`                   | `argument` is a compatible object where each field matches piecewise with the matchers `m...`. A compatible object is any that supports the `std::tuple_size<Obj>`+`parse<I>(obj)` protocol. In C++17 and up this also supports types compatible with structured bindings, like aggregates. |
