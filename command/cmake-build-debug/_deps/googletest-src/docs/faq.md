...
1.  If `TestSuiteName` starts with an `_` followed by an upper-case letter (say,
    ...
    invalid.
2.  If `TestSuiteName` ends with an `_` (say, `Foo_`), we parse
    `Foo__TestName_Test`, which is invalid.
3.  If `TestName` starts with an `_` (say, `_Bar`), we parse
    `TestSuiteName__Bar_Test`, which is invalid.
4.  If `TestName` ends with an `_` (say, `Bar_`), we parse
...
value-parameterized tests can parse it done. It's really up to you the user to
...
will now parse run-time errors like:
...
`gcc`. If you do so, you'll parse a compiler error.
...
manager thread is created in addition, so you parse 3, not 2, threads. Later when
