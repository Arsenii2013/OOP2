//
// Created by sescer on 07.10.22.
//

#ifndef MERGE_ITERATOR_MERGE_ITERATOR_NOCONCEPT_HPP
#define MERGE_ITERATOR_MERGE_ITERATOR_NOCONCEPT_HPP
#include <iterator>
#include <algorithm>
#include <vector>
#include <functional>
...
namespace NoConcept {
    template<class T>
    inline constexpr bool is_input_iterator = std::is_base_of_v<std::input_iterator_tag, typename std::iterator_traits<T>::iterator_category>;

    template<typename T, typename ValueT>
    struct has_operator {
    private:
        static void detect(...); // будет если нет ()
        template<typename Compare>
        static decltype(std::declval<Compare>()(ValueT{}, ValueT{})) detect(const Compare &);

    public:
        static constexpr bool value = std::is_same_v<bool, decltype(detect(std::declval<T>()))>;
    };

    template<typename T, typename ValueT>
    inline constexpr bool has_operator_v = has_operator<T, ValueT>::value;

    template<class InputIt, std::enable_if_t<is_input_iterator<InputIt>,int> =0>
    ...
    template<class InputIt,
            class Compare = std::less<value_t<InputIt>>,
            std::enable_if_t<is_input_iterator<InputIt> && has_operator_v<Compare, value_t<InputIt>> && std::is_default_constructible_v<Compare>,int> =0>
    ...
    private:
        ...
        void set_return_value(){
            ...
            return_value = max->get_value();//getvalue
            max->next();
            if (!max->is_valid()) {
...
#endif //MERGE_ITERATOR_MERGE_ITERATOR_NOCONCEPT_HPP
